...
import Cards, { StockRow } from "./cards";
...
export default function Hero({
  ...
  const runScreener = async (query: string) => {
    const baseUrl =
      process.env.EXPO_PUBLIC_STOCKS_PROXY_URL || "http://localhost:8788";
    ...
    try {
      ...
      if (!response.ok) {
        const message = json?.error || "Unable to run screener";
        throw new Error(message);
      }

      setStocks((json?.rows as StockRow[]) || []);
      setShowStocks(true);
      setStocksError(null);
    } catch (error) {
      setStocksError(
        error instanceof Error ? error.message : "Screener failed",
      );
      setStocks([]);
      setShowStocks(true);
    } finally {
    ...
    >
      {inputOnly ? (
        ...
        >
          ...
          {parseLoading && (
            <Text style={styles.parseText}>Checking your query...</Text>
          )}
